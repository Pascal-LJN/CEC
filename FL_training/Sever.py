...
def cpu_state_dict(model):
    """始终返回 CPU 上的权重 dict（避免 CUDA 序列化导致 CPU-only 客户端报错）"""
...
class Sever(Communicator):
    ...
    def initialize(self, split_layers, offload, first, LR):
        if offload or first:
            ...
            for i in range(len(split_layers)):
                ...
                if split_layers[i] < len(config.model_cfg[self.model_name]) - 1:
                    # Only offloading client need initialize optimizer in server
                    ...
                    cweights = utils.get_model(
                        'Client', self.model_name, split_layers[i], self.device, config.model_cfg).state_dict()
        ...
        # 发 CPU 格式的权重
        msg = ['MSG_INITIAL_GLOBAL_WEIGHTS_SERVER_TO_CLIENT',
               cpu_state_dict(self.uninet)]
    ...
    def train(self, thread_number, client_ips):
        ...
        for i in range(len(client_ips)):
            if config.split_layer[i] == (config.model_len - 1):
                self.threads[client_ips[i]] = threading.Thread(
                    target=self._thread_training_no_offloading, args=(client_ips[i],))
                logger.info(str(client_ips[i]) + ' no offloading training start')
                self.threads[client_ips[i]].start()
            else:
                logger.info(str(client_ips[i]))
                self.threads[client_ips[i]] = threading.Thread(
                    target=self._thread_training_offloading, args=(client_ips[i],))
        ...
        for s in self.client_socks:
            msg = self.recv_msg(
                self.client_socks[s], 'MSG_TRAINING_TIME_PER_ITERATION')
    ...
    def _thread_network_testing(self, client_ip):
        ...
        msg = ['MSG_TEST_NETWORK', cpu_state_dict(self.uninet)]
    ...
    def _thread_training_offloading(self, client_ip):
        ...
        for i in range(iteration):
            msg = self.recv_msg(
                self.client_socks[client_ip], 'MSG_LOCAL_ACTIVATIONS_CLIENT_TO_SERVER')
            smashed_layers = msg[1]
            labels = msg[2]

            inputs, targets = smashed_layers.to(
            ...
            # Send gradients to client (梯度 Tensor，客户端 Communicator 已经兜底 map cpu)
            msg = ['MSG_SERVER_GRADIENTS_SERVER_TO_CLIENT_' +
                   str(client_ip), inputs.grad]
            self.send_msg(self.client_socks[client_ip], msg)
    ...
    def aggregate(self, client_ips):
        ...
        for i in range(len(client_ips)):
            msg = self.recv_msg(
                self.client_socks[client_ips[i]], 'MSG_LOCAL_WEIGHTS_CLIENT_TO_SERVER')
            if config.split_layer[i] != (config.model_len - 1):
                w_local = (utils.concat_weights(self.uninet.state_dict(
                ), msg[1], self.nets[client_ips[i]].state_dict()), config.N / config.K)
                w_local_list.append(w_local)
            else:
                w_local = (msg[1], config.N / config.K)
                w_local_list.append(w_local)
        zero_model = utils.zero_init(self.uninet).state_dict()
        aggregrated_model = utils.fed_avg(
            zero_model, w_local_list, config.N)

        self.uninet.load_state_dict(aggregrated_model)
        return cpu_state_dict(self.uninet)  # 返回 CPU 权重
    ...
    def test(self, r):
        ...
        # Save checkpoint (CPU 格式)
        torch.save(cpu_state_dict(self.uninet),
    ...
    def clustering(self, state, bandwidth):
        ...
        for i in range(len(bandwidth_order)):
            if bandwidth_order[i] < 5:
                # If network speed is limited under 5Mbps, assign to group 2
                labels[i] = 2
    ...
    def action_to_layer(self, action):  # Expanding group actions to each device
        # first calculate cumulated flops
        model_state_flops = []
        cumulated_flops = 0
        ...
        split_layer = []
        for v in action:
            idx = np.where(np.abs(model_flops_list - v) ==
                           np.abs(model_flops_list - v).min())
            idx = idx[0][-1]
    ...
    def concat_norm(self, ttpi, offloading):
        ...
        offloading_order = np.array(
            offloading_order)[np.array(group_max_index)]
    ...
    def get_offloading(self, split_layer):
        ...
        for i in range(len(config.CLIENTS_LIST)):
            ...
            offloading[config.CLIENTS_LIST[i]
                       ] = workload / config.total_flops
            workload = 0
